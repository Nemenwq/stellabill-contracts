...
impl FuzzHarness {
        ...
        let res = match op {
            ...
            Operation::DepositFunds => {
                std::format!(
                    "{:?}",
                    self.client
                        .try_deposit_funds(&self.subscription_id, &address, &5_000_000)
                )
            }
            Operation::CancelSubscription => {
                std::format!(
                    "{:?}",
                    self.client
                        .try_cancel_subscription(&self.subscription_id, &address)
                )
            }
            Operation::PauseSubscription => {
                std::format!(
                    "{:?}",
                    self.client
                        .try_pause_subscription(&self.subscription_id, &address)
                )
            }
            Operation::ResumeSubscription => {
                self.env.mock_all_auths();
                self.client
                    .pause_subscription(&self.subscription_id, &self.subscriber);
                if !is_allowed {
                    self.env.mock_auths(&[]);
                }
                std::format!(
                    "{:?}",
                    self.client
                        .try_resume_subscription(&self.subscription_id, &address)
                )
            }
            Operation::ChargeOneOff => {
                self.env.mock_all_auths();
                let _ = self.client.try_deposit_funds(
                ...
                std::format!(
                    "{:?}",
                    self.client
                        .try_charge_one_off(&self.subscription_id, &address, &1_000_000)
                )
            }
            Operation::WithdrawSubscriberFunds => {
                self.env.mock_all_auths();
                ...
                std::format!(
                    "{:?}",
                    self.client
                        .try_withdraw_subscriber_funds(&self.subscription_id, &address)
                )
            }
            Operation::WithdrawMerchantFunds => {
                self.env.mock_all_auths();
                ...
                let _ = self.client.try_charge_one_off(
                    &self.subscription_id,
                    &self.merchant,
                    &10_000_000,
                );
                ...
                std::format!(
                    "{:?}",
                    self.client
                        .try_withdraw_merchant_funds(&address, &1_000_000)
                )
            }
            Operation::PauseMerchant => {
            ...
            Operation::MerchantRefund => {
                ...
                std::format!(
                    "{:?}",
                    self.client.try_merchant_refund(
                        &address,
                        &self.subscriber,
                        &self.token,
                        &1_000_000
            ...
            Operation::ConfigureUsageLimits => {
                std::format!(
                    "{:?}",
                    self.client.try_configure_usage_limits(
                        &address,
                        &self.subscription_id,
                        &Some(100),
                        &3600,
            ...
            Operation::PartialRefund => {
                ...
                std::format!(
                    "{:?}",
                    self.client.try_partial_refund(
                        &address,
                        &self.subscription_id,
                        &self.subscriber,
                        &1_000_000
            ...
            Operation::BatchCharge => {
                if is_allowed {
                    ...
                    let batch: SorobanVec<u32> =
                        SorobanVec::from_array(env, [self.subscription_id]);
            ...
            Operation::AddAcceptedToken => {
                ...
                std::format!(
                    "{:?}",
                    self.client
                        .try_add_accepted_token(&address, &other_token, &6)
                )
            }
            Operation::RemoveAcceptedToken => {
                let other_token = Address::generate(env);
                self.env.mock_all_auths();
                self.client
                    .add_accepted_token(&self.admin, &other_token, &6);
                if !is_allowed {
                    self.env.mock_auths(&[]);
                }
                std::format!(
                    "{:?}",
                    self.client
                        .try_remove_accepted_token(&address, &other_token)
                )
            }
            Operation::SetSubscriberCreditLimit => {
                std::format!(
                    "{:?}",
                    self.client.try_set_subscriber_credit_limit(
                        &address,
                        &self.subscriber,
                        &self.token,
                        &100_000_000
    ...
    pub fn is_allowed(&self, op: Operation, caller: Role) -> bool {
        match op {
            ...
            | Operation::RemoveAcceptedToken
            | Operation::SetSubscriberCreditLimit
            | Operation::ExportContractSnapshot => caller == Role::Admin,
            ...
            | Operation::PauseMerchant
            | Operation::UnpauseMerchant
            | Operation::ConfigureUsageLimits => caller == Role::Merchant,
    ...
    for &op in Operation::all() {
        for &role in Role::all() {
            ...
            match (expected_allowed, result) {
                ...
                (true, Err(e)) => {
                    std::panic!(
                        "Operation {:?} should be allowed for role {:?}, but failed! Outcome: {}",
                        op,
                        role,
                        e
                    );
                ...
                (false, Ok(())) => {
                    std::panic!(
                        "Operation {:?} should NOT be allowed for role {:?}, but succeeded!",
                        op,
                        role
                    );
                }
...
fn test_admin_rotation_edge_case() {
    ...
    assert!(
        res.is_err(),
        "Old admin should no longer be authorized after rotation"
    );

    let res_new = harness.client.try_set_min_topup(&new_admin, &4_000_000);
    assert!(
        res_new.is_ok(),
        "New admin should be authorized after rotation"
    );
}
