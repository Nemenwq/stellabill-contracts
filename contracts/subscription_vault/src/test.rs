...
fn test_admin_rotation_affects_recovery_operations() {
    ...
    test_env.client.recover_stranded_funds(
        ...
        &recipient,
        &1_000_000i128,
        &String::from_str(&test_env.env, "admin-recovery-1"),
        &RecoveryReason::UserOverpayment,
    );
...
fn test_all_admin_operations_after_rotation() {
    ...
    test_env.client.recover_stranded_funds(
        ...
        &recipient,
        &1_000_000i128,
        &String::from_str(&test_env.env, "admin-recovery-2"),
        &RecoveryReason::UserOverpayment,
    );
...
fn test_pause_with_varying_intervals() {
    ...
    let id1 = test_env
        .client
        .create_subscription(&s1, &m, &AMOUNT, &daily, &false, &None::<i128>, &None);
    let id2 = test_env
        .client
        .create_subscription(&s2, &m, &AMOUNT, &weekly, &false, &None::<i128>, &None);
    let id3 =
        test_env
            .client
            .create_subscription(&s3, &m, &AMOUNT, &monthly, &false, &None::<i128>, &None);
...
fn test_oneoff_lifetime_cap_boundary() {
    ...
    client.charge_one_off(&id, &merchant, &19_000_000i128);
    let sub = client.get_subscription(&id);
    assert_eq!(sub.lifetime_charged, 19_000_000);

    // Any further charge that exceeds the cap should fail
    let res = client.try_charge_one_off(&id, &merchant, &2_000_000i128);
