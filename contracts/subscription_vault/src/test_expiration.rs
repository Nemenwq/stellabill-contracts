...
fn test_cleanup_and_archival() {
    ...
    let sub_id = client.create_subscription_with_token(&subscriber, &merchant, &token.address, &1_000_000, &60, &false, &None::<i128>, &Some(T0 + 2 * INTERVAL));
...
fn test_expiration_vs_cancellation() {
    ...
    let sub_id1 = client.create_subscription_with_token(&subscriber, &merchant, &token.address, &1_000_000, &60, &false, &None::<i128>, &Some(T0 + 2 * INTERVAL));
    ...
    let sub_id2 = client.create_subscription_with_token(&subscriber, &merchant, &token.address, &1_000_000, &60, &false, &None::<i128>, &Some(T0 + 2 * INTERVAL));
